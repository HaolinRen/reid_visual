...
var map3D = function() {
	...
	function createRandom() {
		...
		for (let i = 0; i < xNum; i += 1) {
			...
			for (let j = 0; j < zNum; j += 1) {
				...
				let roadWidth = Math.random() * 10 + 2;
	...
	return {
		...
		drawLine : function(points, color) {
			...
			var vpoints = [];
			for (let p of points) {
				vpoints.push( new THREE.Vector3(p[0], p[1], p[2]) ) 
			}
			var geometry = new THREE.BufferGeometry().setFromPoints( vpoints );
